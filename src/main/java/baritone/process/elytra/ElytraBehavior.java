...
public final class ElytraBehavior /*implements Helper*/ {
    ...
    public final class PathManager {
        ...
        public void tick() {
            ...
            if (this.maxPlayerNear == prevMaxNear && ctx.baritonePlayer().getEntity().isFallFlying()) {
                    ...
                    .whenComplete((result, ex) -> {
                        ...
                        if (ex != null) {
                            ...
                            if (cause instanceof PathCalculationException) {
                                ...
                                if (ctx.baritonePlayer().getEntity().distanceToSqr(pathStart.getCenter()) < 16 * 16) {
    ...
    public void onRenderPass(RenderEvent event) {
        ...
        if (this.simulationLine != null && Baritone.settings().elytraRenderSimulation.value) {
            ...
            final Vec3 offset = ctx.baritonePlayer().getEntity().getPosition(event.getPartialTicks());
    ...
    public void pathTo() {
        if (!Baritone.settings().elytraAutoJump.value || ctx.baritonePlayer().getEntity().isFallFlying()) {
    ...
    public void onTick() {
        ...
        if ((now - this.timeLastCacheCull) / 1000 > Baritone.settings().elytraTimeBetweenCacheCullSecs.value) {
            this.context.queueCacheCulling(ctx.baritonePlayer().getEntity().chunkPosition().x, ctx.baritonePlayer().getEntity().chunkPosition().z, Baritone.settings().elytraCacheCullDistance.value, this.boi);
    ...
    public void tick() {
        ...
        if (ctx.baritonePlayer().getEntity().horizontalCollision) {
            logVerbose("hbonk");
        }
        if (ctx.baritonePlayer().getEntity().verticalCollision) {
            logVerbose("vbonk");
        }
        ...
        final boolean inLava = ctx.baritonePlayer().getEntity().isInLava();
        if (inLava) {
    ...
    private void tickUseFireworks(final Vec3 start, final Vec3 goingTo, final boolean isBoosted, final boolean forceUseFirework) {
        ...
        final boolean useOnDescend = !Baritone.settings().elytraConserveFireworks.value || ctx.baritonePlayer().getEntity().position().y < goingTo.y + 5;
        final double currentSpeed = new Vec3(
                ctx.baritonePlayer().getEntity().getDeltaMovement().x,
                // ignore y component if we are BOTH below where we want to be AND descending
                ctx.baritonePlayer().getEntity().position().y < goingTo.y ? Math.max(0, ctx.baritonePlayer().getEntity().getDeltaMovement().y) : ctx.baritonePlayer().getEntity().getDeltaMovement().y,
                ctx.baritonePlayer().getEntity().getDeltaMovement().z
        ).lengthSqr();
        ...
        if (this.remainingFireworkTicks <= 0 && (forceUseFirework || (!isBoosted
                && useOnDescend
                && (ctx.baritonePlayer().getEntity().position().y < goingTo.y - 5 || start.distanceTo(new Vec3(goingTo.x + 0.5, ctx.baritonePlayer().getEntity().position().y, goingTo.z + 0.5)) > 5) // UGH!!!!!!!
    ...
    private final class SolverContext {
        ...
        public SolverContext(boolean async) {
            ...
            this.boundingBox = ctx.baritonePlayer().getEntity().getBoundingBox();
            this.ignoreLava = ctx.baritonePlayer().getEntity().isInLava();
    ...
    private Optional<FireworkRocketEntity> getAttachedFirework() {
        return ctx.entitiesStream()
                ...
                .filter(x -> Objects.equals(((IFireworkRocketEntity) x).getBoostedEntity(), ctx.baritonePlayer()))
    ...
    public boolean clearView(Vec3 start, Vec3 dest, boolean ignoreLava) {
        ...
        if (!ignoreLava) {
            ...
            clear = ctx.world().clip(new ClipContext(start, dest, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, ctx.baritonePlayer().getPlayer())).getType() == HitResult.Type.MISS;
        }
    ...
    private void trySwapElytra() {
        ...
        if (goodElytraSlot != -1) {
            ...
            queueWindowClick(ctx.baritonePlayer().getPlayer().inventoryMenu.containerId, slotId, 0, ClickType.PICKUP);
            queueWindowClick(ctx.baritonePlayer().getPlayer().inventoryMenu.containerId, CHEST_SLOT, 0, ClickType.PICKUP);
            queueWindowClick(ctx.baritonePlayer().getPlayer().inventoryMenu.containerId, slotId, 0, ClickType.PICKUP);
        }
    }
