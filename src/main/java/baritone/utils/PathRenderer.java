...
public final class PathRenderer implements IRenderer {
    ...
    public static void render(RenderEvent event, PathingBehavior behavior) {
        ...
        if (current != null && settings.renderSelectionBoxes.value) {
            drawManySelectionBoxes(event.getModelViewStack(), ctx.baritonePlayer().getEntity(), current.toBreak(), settings.colorBlocksToBreak.value);
            drawManySelectionBoxes(event.getModelViewStack(), ctx.baritonePlayer().getEntity(), current.toPlace(), settings.colorBlocksToPlace.value);
            drawManySelectionBoxes(event.getModelViewStack(), ctx.baritonePlayer().getEntity(), current.toWalkInto(), settings.colorBlocksToWalkInto.value);
        }
        ...
        behavior.getInProgress().ifPresent(currentlyRunning -> {
            ...
            currentlyRunning.pathToMostRecentNodeConsidered().ifPresent(mr -> {
                ...
                drawManySelectionBoxes(event.getModelViewStack(), ctx.baritonePlayer().getEntity(), Collections.singletonList(mr.getDest()), settings.colorMostRecentConsidered.value);
            });
        });
    }
    ...
    private static void drawGoal(PoseStack stack, IPlayerContext ctx, Goal goal, float partialTicks, Color color, boolean setupRender) {
        ...
        if (goal instanceof IGoalRenderPos) {
            ...
            minX = ctx.baritonePlayer().getEntity().position().x - settings.yLevelBoxSize.value - renderPosX;
            minZ = ctx.baritonePlayer().getEntity().position().z - settings.yLevelBoxSize.value - renderPosZ;
            maxX = ctx.baritonePlayer().getEntity().position().x + settings.yLevelBoxSize.value - renderPosX;
            maxZ = ctx.baritonePlayer().getEntity().position().z + settings.yLevelBoxSize.value - renderPosZ;
